...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function autofillChildFieldsForRow(tr) {
    ...
    const info = await fetchChildDetails(centre, child);
    if (!info) return;
    const cs = tr.querySelector('[name="Child Status"]');
    const fs = tr.querySelector('[name="Family Status"]');
    const bc = tr.querySelector('[name="Billing Cycle"]');
    if (cs) cs.value = info["Child Status"] || "";
    if (fs) fs.value = info["Family Status"] || "";
    if (bc) bc.value = info["Billing Cycle"] || "";
  }
